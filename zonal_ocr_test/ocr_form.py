...
def cleanup_text(text):
    #strip out the non ASCII text so we can draw the text on the image using OpenCV
    return "".join([c if ord(c) < 128 else "" for c in text]).strip()

#construct the argument parser and parse the arguments
ap = argparse.ArgumentParser()
ap.add_argument("-i", "--image", required=True,
    help="path to input image that we'll align to the template")
ap.add_argument("-t", "--template", required=True,
    help="path to input template image")
args = vars(ap.parse_args())

#create a named tuple which we can use to create locations of the input document which we want to OCR
OCRLocation = namedtuple("OCRLocation", ["id", "bbox", "filter_keywords"])

#define teh locations of each area of the document we wish to OCR
OCR_LOCATIONS = [
    OCRLocation("step1_full_names", (386, 36, 436, 139), ["full", "name"]),
    OCRLocation("step1_id_number", (190, 30, 557, 97), ["id", "number"]),
]
