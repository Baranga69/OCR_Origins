...
def align_images(image, template, maxfeatures=500, keepPercent=0.2, debug=False):
    ...
    #allocate memory for the keypoints (x,y) - coordinates from the top matches--we'll use these coordinates to compute our homographic matrix
    ...
    for (i,m) in enumerate(matches):
        ...
        ptsB[i] = kpsB[m.queryIdx].pt

    #compute the homography matrix between the two sets of matched points
    (H, mask) = cv2.findHomography(ptsA, ptsB, method=cv2.RANSAC)

    #use the homography matrix to align the image 
